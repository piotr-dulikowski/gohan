...
const (
	...
	attachedNestedPluralURL   = baseURL + "/v2.0/nested_attachers"
	...
	Describe("Attach policy tests", func() {
		...
		Describe("Relation property nested in another", func() {
			...
			DescribeTable("Owner should be able to attach",
				...
				Entry("Relation inside an object", attachedNestedPluralURL, &relationInsideAnObject),
				Entry("Relation inside an object via wildcard attacher", attacherWildcardPluralURL, &relationInsideAnObject),
				Entry("Relation inside an array", attachedNestedPluralURL, &relationInsideAnArray),
